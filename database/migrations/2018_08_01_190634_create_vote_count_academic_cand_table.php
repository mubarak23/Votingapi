...
class CreateVoteCountAcademicCandTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('vote_count_academic_cand', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('students_id')->references('id')->on('students');
            $table->string('student_fullname')->references('full_name')->on('students');
            $table->integer('vote');
            $table->integer('candidate_id')->references('id')->on('academic_cands');
    ...
    {
        Schema::dropIfExists('vote_count_academic_cand');
    }
}
